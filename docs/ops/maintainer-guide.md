...
accepted, which gates every change runs, and which operational document
...
- Compatibility and artifact identity: the qualification runbook.
- Clean-clone CI, Plugin Doctor, fresh-session inventory: the clean-clone
  runbook.
- Installation, enablement, disablement, removal: the installation
  lifecycle runbook.
...
- Release candidate preparation: the release handoff runbook.

Maintainers add a new operational document only through a roadmap work
unit that names its owner; `docs/ops/README.md` remains the single index.
